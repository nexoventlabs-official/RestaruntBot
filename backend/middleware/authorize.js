...
function authorize(...allowedRoles) {
  return (req, res, next) => {
    ...
    if (!allowedRoles.includes(userRole)) {
      console.warn(`⚠️ Authorization failed: ${userRole} tried to access ${req.method} ${req.originalUrl}`);
      return res.status(403).json({ 
    ...
    console.log(`✅ Authorized: ${userRole} - ${req.method} ${req.originalUrl}`);
...
function authorizeOwner(getResourceOwnerId) {
  return (req, res, next) => {
    ...
    if (resourceOwnerId !== userId && resourceOwnerId !== userId.toString()) {
      console.warn(`⚠️ Ownership check failed: User ${userId} tried to access resource owned by ${resourceOwnerId}`);
      return res.status(403).json({ 
