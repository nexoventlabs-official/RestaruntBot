...
async function recordRequest(messageType, route = 'unknown') {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record request:', error.message);
  }
}
...
async function recordSuccess() {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record success:', error.message);
  }
}
...
async function recordFailure() {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record failure:', error.message);
  }
}
...
async function recordResponseTime(component, durationMs, detail = null) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record response time:', error.message);
  }
}
...
async function recordDomainAction(domain, action, success = true) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record domain action:', error.message);
  }
}
...
async function recordApiCall(service, success = true, durationMs = 0) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record API call:', error.message);
  }
}
...
async function recordEvent(event) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record event:', error.message);
  }
}
...
async function recordError(errorType, errorMessage) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to record error:', error.message);
  }
}
...
async function getMetrics() {
  ...
  try {
    ...
    const [
      ...
      redis.keys(`${METRICS_PREFIX}requests:type:*`),
      redis.keys(`${METRICS_PREFIX}requests:route:*`),
      redis.keys(`${METRICS_PREFIX}domain:*`),
      redis.keys(`${METRICS_PREFIX}api:*`),
      redis.keys(`${METRICS_PREFIX}event:*`),
    ...
    console.error('❌ [Metrics] Failed to get metrics:', error.message);
...
async function getResponseTimeStats(component, detail = null) {
  ...
  try {
    ...
    console.error('❌ [Metrics] Failed to get response time stats:', error.message);
    return { error: error.message };
  }
}
...
async function resetMetrics() {
  ...
  try {
    ...
    console.log(`✅ [Metrics] Reset ${keys.length} metric keys`);
  } catch (error) {
    console.error('❌ [Metrics] Failed to reset metrics:', error.message);
  }
}
