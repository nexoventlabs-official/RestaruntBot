...
async function retryWithBackoff(fn, retries = MAX_RETRIES, delay = INITIAL_DELAY) {
  try {
    ...
    console.log(`⚠️ [Sheets Retry] Attempt failed, retrying in ${delay}ms... (${retries} retries left)`);
...
async function addOrderReliable(order) {
  ...
  try {
    ...
    console.error('❌ [Sheets] Failed to add order after retries:', error.message);
...
async function updateOrderReliable(orderId, updates) {
  ...
  try {
    ...
    console.error('❌ [Sheets] Failed to update order after retries:', error.message);
...
async function addCustomerReliable(customer) {
  ...
  try {
    ...
    console.error('❌ [Sheets] Failed to add customer after retries:', error.message);
...
async function retryFailedSyncs() {
  if (syncErrors.size === 0) {
    console.log('✅ [Sheets] No failed syncs to retry');
    return { retried: 0, succeeded: 0, failed: 0 };
  }
  
  console.log(`🔄 [Sheets] Retrying ${syncErrors.size} failed syncs...`);
  ...
  for (const [key, errorInfo] of syncErrors.entries()) {
    try {
      ...
      if (errorInfo.operation === 'addOrder') {
        ...
        console.log(`⏭️ [Sheets] Skipping retry for ${errorInfo.orderId} (needs manual intervention)`);
        failed++;
      ...
      console.error(`❌ [Sheets] Retry failed for ${errorInfo.orderId}:`, error.message);
      failed++;
    }
  }
...
function clearSyncErrors() {
  ...
  syncErrors.clear();
  console.log(`✅ [Sheets] Cleared ${count} sync errors`);
  return count;
}
