 ...
 * - Handle webhook events (payment.captured)
...
async function handlePaymentSuccess(params) {
  ...
  try {
    ...
    // Pre-compute state machine transition
    const transition = transitionStatus(order, 'confirmed', 'Payment received - Order confirmed', 'payment');
    if (!transition.success) {
      logger.error('Payment transition failed', { orderId, from: order.status, reason: transition.reason });
      return { success: false, error: `Status transition failed: ${transition.reason}` };
    }
    ...
    if (!updatedOrder) {
      logger.warn('Payment already processed by another endpoint', { orderId });
      return { success: true, message: 'Order already paid', order };
    }
    
    // Update customer atomically
    await Customer.findOneAndUpdate(
      { phone: updatedOrder.customer.phone, hasOrdered: { $ne: true } },
      { $set: { hasOrdered: true } }
    );
    ...
    googleSheets.updateOrderStatus(updatedOrder.orderId, updatedOrder.status, updatedOrder.paymentStatus).catch(err => 
    ...
    if (updatedOrder.customer.email) {
      await sendEmailConfirmation(updatedOrder).catch(err =>
...
async function handleWebhookPaymentCaptured(payload) {
  try {
    ...
    // Pre-compute state machine transition
    const transition = transitionStatus(order, 'confirmed', 'Payment captured - Order confirmed', 'webhook');
    if (!transition.success) {
      logger.error('Webhook payment transition failed', { orderId: order.orderId, from: order.status, reason: transition.reason });
      return { success: false, error: `Status transition failed: ${transition.reason}` };
    }
