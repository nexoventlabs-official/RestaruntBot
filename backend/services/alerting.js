...
async function sendEmailAlert(title, message, severity = 'error', metadata = {}) {
  ...
  try {
    ...
    console.error('❌ [Alerting] Failed to send email alert:', error.message);
    return false;
  }
}
...
async function sendAlert(title, message, severity = 'error', metadata = {}) {
  ...
  if (!shouldSendAlert(alertKey)) {
    console.log(`⏭️ [Alerting] Skipping alert (cooldown): ${title}`);
    return;
  }
  
  console.log(`📢 [Alerting] Sending alert: ${title}`);
  ...
  if (!anySuccess && (ENABLE_SLACK || ENABLE_EMAIL)) {
    console.warn('⚠️ [Alerting] All alert channels failed, check configuration');
  }
}
