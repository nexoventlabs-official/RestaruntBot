...
async function explainQuery(model, query, options = {}) {
  try {
    ...
    if (stats.executionTimeMs > SLOW_QUERY_THRESHOLD) {
      console.warn(`⚠️ [Query Optimizer] Slow query detected: ${stats.executionTimeMs}ms`);
    }
    
    // Alert if collection scan
    if (stats.needsIndex) {
      console.warn(`⚠️ [Query Optimizer] Collection scan detected, consider adding index`);
    }
    
    return stats;
  } catch (error) {
    console.error('❌ [Query Optimizer] Explain error:', error.message);
...
async function analyzeQuery(model, query, options = {}) {
  try {
    ...
    console.error('❌ [Query Optimizer] Analyze error:', error.message);
    return { error: error.message };
  }
}
...
async function getIndexRecommendations(modelName) {
  try {
    ...
    console.error('❌ [Query Optimizer] Get recommendations error:', error.message);
    return { error: error.message };
  }
}
...
async function monitorQuery(model, query, operation = 'find') {
  ...
  try {
    ...
    if (executionTime > SLOW_QUERY_THRESHOLD) {
      console.warn(`⚠️ [Query Monitor] Slow ${operation} on ${model.modelName}: ${executionTime}ms`);
    ...
    const executionTime = Date.now() - startTime;
    console.error(`❌ [Query Monitor] ${operation} error on ${model.modelName}: ${error.message} (${executionTime}ms)`);
    throw error;
  }
}
...
async function batchInsert(model, documents, batchSize = 1000) {
  try {
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      console.log(`✅ [Query Optimizer] Batch inserted ${inserted.length} documents`);
    }
    
    return results;
  } catch (error) {
    console.error('❌ [Query Optimizer] Batch insert error:', error.message);
    throw error;
  }
}
...
async function optimizedAggregate(model, pipeline, options = {}) {
  try {
    ...
    if (executionTime > SLOW_QUERY_THRESHOLD) {
      console.warn(`⚠️ [Query Optimizer] Slow aggregation on ${model.modelName}: ${executionTime}ms`);
    }
    
    return result;
  } catch (error) {
    console.error('❌ [Query Optimizer] Aggregate error:', error.message);
    throw error;
  }
}
